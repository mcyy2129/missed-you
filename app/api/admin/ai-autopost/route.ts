...
export async function POST(req: NextRequest) {
  try {
    ...
    let persona;
    if (personaId) {
      persona = AI_PERSONAS.find(p => p.id === personaId);
    } else {
    ...
    return NextResponse.json({ error: 'AI发帖失败' }, { status: 500 });
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ created: results.length, posts: results });
  } catch (error) {
    console.error('AI batch auto post error:', error);
    return NextResponse.json({ error: 'AI批量发帖失败' }, { status: 500 });
  }
}
