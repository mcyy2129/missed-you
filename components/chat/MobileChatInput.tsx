...
export default function MobileChatInput({ onSend }: MobileChatInputProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200/60 safe-area-bottom">
      <AnimatePresence>
        {showEmoji && (
          <motion.div
      ...
      <div className="flex items-end gap-2 px-3 py-2 max-w-lg mx-auto">
        <motion.button
          ...
          className={`p-2 rounded-full transition-colors ${
            showEmoji ? 'bg-rose-100 text-rose-500' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
